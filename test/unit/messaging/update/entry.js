...
describe("UPDATE entry", ()=> {
  ...
  describe("validate - direct caching", ()=> {
    it("should validate correct entry value", ()=> {
      ...
      assert.equal(entry.validateDirectCacheProcess({filePath: "testing"}), false);
      assert.equal(entry.validateDirectCacheProcess({filePath: "testing", data: {"test": "test"}}), false);
      assert.equal(entry.validateDirectCacheProcess({filePath: "testing", data: {"test": "test"}, from: ""}), false);
      assert.equal(entry.validateDirectCacheProcess({filePath: "testing", data: {"test": "test"}, from: "testing"}), true);
    });
  });
